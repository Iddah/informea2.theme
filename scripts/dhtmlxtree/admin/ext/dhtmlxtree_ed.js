...
dhtmlXTreeObject.prototype._stopEdit = function (a) {
	if (this._editCell) {
		...
		if (this._editCell.id != a) {
			...
			if (editText === true)editText = this._editCell.span.childNodes[0].value; else if (editText === false)editText = this._editCell._oldValue;
...
dhtmlXTreeObject.prototype._editItem = function (id, tree) {
	if (this._eItEd) {
		...
		if (editText === true)editText = temp.label; else if (editText === false)return;
