	...
	for (var i = 0; i < tag.childNodes.length; i++) {
		...
		if ((z.nodeType == 1) && (z.tagName.toLowerCase() == "li")) {
			...
			for (var j = 0; j < z.childNodes.length; j++) {
				...
				if (zc.nodeType == 3)c += zc.data; else if (zc.tagName.toLowerCase() != "ul") c += dhx_outer_html(zc); else ul = zc;
			}
			;
...
if (window.addEventListener)window.addEventListener("load", dhx_init_trees, false); else if (window.attachEvent)window.attachEvent("onload", dhx_init_trees);
