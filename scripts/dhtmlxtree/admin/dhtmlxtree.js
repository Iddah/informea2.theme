...
dhtmlXTreeObject.prototype._attachChildNode = function (parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode, afterNode) {
	...
	if ((beforeNode) && (beforeNode.tr.nextSibling))
		parentObject.htmlNode.childNodes[0].insertBefore(tr, beforeNode.tr.nextSibling); else if (this.parsingOn == parentObject.id) {
...
dhtmlXTreeObject.prototype._createItem = function (acheck, itemObject, mode) {
	...
	if (_isIE && _isIE > 7)td2.style.width = "999999px"; else if (!window._KHTMLrv)td2.style.width = "100%";
...
dhtmlXTreeObject.prototype._getAllScraggyItems = function (node) {
	...
	for (var i = 0; i < node.childsCount; i++) {
		...
		} else if (!z)z = node.childNodes[i].id; else z += this.dlmtr + node.childNodes[i].id;
	}
	;
	return z;
};
...
dhtmlXTreeObject.prototype.moveItem = function (itemId, mode, targetId, targetTree) {
	...
	switch (mode) {
		...
		case "down_strict":
			...
			if (z == count)this._moveNodeTo(sNode, sNode.parentObject); else if (z < count)this._moveNodeTo(sNode, sNode.parentObject, sNode.parentObject.childNodes[z + 2]);
			break;
		case "down":
...
(function () {
	dhtmlx.extend_api("dhtmlXTreeObject", {_init: function (obj) {
