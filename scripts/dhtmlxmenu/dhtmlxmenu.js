...
function dhtmlXMenuObject(b, c) {
	...
	if (b == null)this.base = document.body; else {
	...
	this._removeSubItemFromSelected = function (a, b) {
		...
		if (d == !0)this._selectedSubItems =
			c;
		return d
	};
	this._getSubItemToDeselectByPolygon = function (a) {
		for (var b = [], c = 0; c < this._selectedSubItems.length; c++)if (this._selectedSubItems[c][1] == a) {
	...
	this._hidePolygon = function (a) {
		if (this.idPull["polygon_" + a] != null)if (this.sxDacProc != null && this.idPull["sxDac_" + a] != null)this.idPull["sxDac_" + a]._hide(); else if (this.idPull["polygon_" + a].style.display != "none") {
			...
			if (this._isIE6 && this.idPull["polygon_" + a + "_ie6cover"] != null)this.idPull["polygon_" + a + "_ie6cover"].style.display = "none";
	...
	this._showPolygon = function (a, b) {
		...
		if (c != 0) {
			...
			if (this.idPull[d] != null && this.idPull[a] != null && (!this.menuModeTopLevelTimeout || this.menuMode != "web" || this.context || this.idPull[a]._mouseOver || b != this.dirTopLevel)) {
				...
				if (this.itemPull[a] != null)var C = "polygon_" + this.itemPull[a].parent; else this.context && (C = this.idPull[this.idPrefix + this.topId]);
				var q = this.idPull[a].tagName != null ? getAbsoluteLeft(this.idPull[a]) : this.idPull[a][0],
					t = this.idPull[a].tagName != null ? getAbsoluteTop(this.idPull[a]) : this.idPull[a][1], u = this.idPull[a].tagName != null ? this.idPull[a].offsetWidth : 0, B = this.idPull[a].tagName != null ? this.idPull[a].offsetHeight + e + g : 0, l = 0, o = 0, s = this.idPull[d].offsetWidth, x = this.idPull[d].offsetHeight;
				...
				b == "left" && (l = q - this.idPull[d].offsetWidth +
					2, o = t + 2);
				b == "top" && (l = q - 1, o = t - x + 2);
				if (this.fixedPosition)var v = 65536, y = 65536; else if (v = this.menuX2 != null ? this.menuX2 : 0, y = this.menuY2 != null ? this.menuY2 : 0, v == 0)window.innerWidth ? (v = window.innerWidth, y = window.innerHeight) : (v = document.body.offsetWidth, y = document.body.scrollHeight);
				...
				if (this.sxDacProc != null && this.idPull["sxDac_" + a] != null)this.idPull["sxDac_" + a]._show(); else {
					this.idPull[d].style.visibility = "";
					if (this.limit > 0 && this.limit < c)f.style.left = l + "px", f.style.top = o + "px", f.style.width = s + this._arrowFFFix + "px", f.style.visibility = "", i.style.left = l + "px", i.style.top = o + e + x + "px", i.style.width = s + this._arrowFFFix + "px", i.style.visibility = "", this._checkArrowsState(a);
					if (this._isIE6) {
						var r = d + "_ie6cover";
						if (this.idPull[r] ==
							null) {
	...
	this._redistribSubLevelSelection = function (a, b) {
		...
		for (var d = 0; d < c.length; d++)if (this.idPull[c[d]] != null && c[d] != a && this.itemPull[c[d]].state == "enabled")this.idPull[c[d]].className = "sub_item";
		for (d = 0; d < this._openedPolygons.length; d++)this._openedPolygons[d] !=
			b && this._hidePolygon(this._openedPolygons[d]);
		if (this.itemPull[a].state == "enabled") {
			...
			if ((this.itemPull[a].complex ||
				this.dLoad && this.itemPull[a].loaded == "yes") && this.itemPull[a].complex && this.idPull["polygon_" + a] != null)this._updateItemComplexState(a, !0, !0), this._showPolygon(a, this.dirSubLevel);
	...
	this._doOnClick = function (a, b, c) {
		...
		if (this.itemPull[this.idPrefix + a].href_link != null && this.itemPull[this.idPrefix + a].state == "enabled") {
			...
			d.action = e[0];
			if (e[1] != null)for (var f = String(e[1]).split("&"), g = 0; g < f.length; g++) {
		...
		} else b.charAt(0) != "c" && b.charAt(1) != "d" && b.charAt(2) != "s" && (this.checkEvent("onClick") ? (this._clearAndHide(),
			this._isContextMenuVisible() && this.contextAutoHide && this._hideContextMenu(), this.callEvent("onClick", [a, this.contextMenuZoneId, c])) : b.charAt(1) == "d" || this.menuMode == "win" && b.charAt(2) == "t" || (this._clearAndHide(), this._isContextMenuVisible() && this.contextAutoHide && this._hideContextMenu()))
	};
	...
	this._searchMenuNode = function (a, b) {
		for (var c = [], d = 0; d < b.length; d++)if (typeof b[d] ==
			"object") {
		...
		for (var b = 0; b < a._selectedSubItems.length; b++) {
			...
			if (a.idPull[c] != null && a.itemPull[c].state == "enabled") {
				...
				if (a.idPull[c].className ==
					"dhtmlxMenu_" + a.skin + "_TopLevel_Item_Selected")a.idPull[c].className = a.itemPull[c].cssNormal != null ? a.itemPull[c].cssNormal : "dhtmlxMenu_" + a.skin + "_TopLevel_Item_Normal"
			}
			a._hidePolygon(c)
		}
		a.menuTouched = !1;
	...
	this._buildMenu = function (a, b) {
		for (var c = 0, d = 0; d < a.childNodes.length; d++)if (a.childNodes[d].tagName == this.itemTagName) {
			...
			if (f.type ==
				"checkbox")f.checked = e.getAttribute("checked") != null, f.imgen = "chbx_" + (f.checked ? "1" : "0"), f.imgdis = f.imgen;
			...
			f.state = e.getAttribute("enabled") != null || e.getAttribute("disabled") != null ? e.getAttribute("enabled") == "false" || e.getAttribute("disabled") ==
			...
			for (var g = 0; g < e.childNodes.length; g++) {
				...
				if (i == this.userDataTagName) {
					...
					n.getAttribute("name") != null && (this.userData[f.id + "_" + n.getAttribute("name")] = n.firstChild.nodeValue != null ? n.firstChild.nodeValue :
						"")
				}
	...
	this._xmlParser = function () {
		if (a.dLoad) {
			...
			parentId =
				b.getAttribute("parentId") != null ? b.getAttribute("parentId") : null;
			if (parentId == null)a._buildMenu(b, null), a._initTopLevelMenu(); else {
	...
	_isIE ? (document.body.attachEvent("onclick",
		this._bodyClick), document.body.attachEvent("oncontextmenu", this._bodyContext)) : (window.addEventListener("click", this._bodyClick, !1), window.addEventListener("contextmenu", this._bodyContext, !1));
...
dhtmlXMenuObject.prototype._updateItemComplexState = function (b, c) {
	if (!this.context && this._getItemLevelType(b.replace(this.idPrefix, "")) == "TopLevel")this.itemPull[b].complex = c; else if (!(this.idPull[b] == null || this.itemPull[b] == null))if (this.itemPull[b].complex = c, b != this.idPrefix + this.topId) {
...
dhtmlXMenuObject.prototype._redistribTopLevelSelection = function (b) {
	...
	this._removeSubItemFromSelected(-1, -1);
	for (var a = 0; a < c.length; a++)if (c[a] != b && this._hidePolygon(c[a]), this.idPull[c[a]] != null && c[a] != b)this.idPull[c[a]].className = this.idPull[c[a]].className.replace(/Selected/g, "Normal");
...
dhtmlXMenuObject.prototype._renderToplevelItem = function (b, c) {
	...
	d.onmouseover = function () {
		...
		for (var c = 0; c < b.length; c++)if (b[c] != this.id && a._hidePolygon(b[c]), a.idPull[b[c]] != null && b[c] != this.id)if (a.itemPull[b[c]].cssNormal != null)a.idPull[b[c]].className = a.itemPull[b[c]].cssNormal; else {
		...
		if (a.itemPull[this.id].state == "enabled") {
			...
			if ((!a.dLoad || a.dLoad && (!a.itemPull[this.id].loaded || a.itemPull[this.id].loaded == "yes")) && a.itemPull[this.id].complex && a.menuSelected != -1)if (a.menuModeTopLevelTimeout && a.menuMode == "web" && !a.context) {
				this._mouseOver = !0;
				var e = this.id;
	...
	d.onclick = function (b) {
		...
		if (!(a.menuMode != "web" && a.itemPull[this.id].state == "disabled")) {
			...
			var d = a.itemPull[this.id].complex ? "c" : "-", e = a.itemPull[this.id].state != "enabled" ? "d" : "-", f = {ctrl: b.ctrlKey,
				alt: b.altKey, shift: b.shiftKey};
...
dhtmlXMenuObject.prototype._updateItemImage = function (b) {
	...
	if (c)for (var d = 0; d < this.idPull[b].childNodes.length; d++)try {
	...
	if (g.length > 0)if (a != null)a.src = this.imagePath + g; else if (c)a = document.createElement("IMG"), a.className = "dhtmlxMenu_TopLevel_Item_Icon", a.src = this.imagePath + g, a.border = "0", a.id = "image_" + b, !this._rtl && this.idPull[b].childNodes.length > 0 ? this.idPull[b].insertBefore(a, this.idPull[b].childNodes[0]) : this.idPull[b].appendChild(a); else {
...
dhtmlXMenuObject.prototype.removeItem = function (b, c, a) {
	...
	if (b != this.idPrefix + this.topId) {
		...
		if (f == "separator") {
			...
			this.itemPull[b].parent == this.idPrefix + this.topId ? (e.onclick = null, e.onselectstart = null, e.id = null) : (e.childNodes[0].childNodes[0].onclick = null, e.childNodes[0].childNodes[0].onselectstart = null, e.childNodes[0].childNodes[0].id = null, e.childNodes[0].removeChild(e.childNodes[0].childNodes[0]),
	...
	for (var j = 0; j < h.length; j++)if (this.idPull["polygon_" + h[j]]) {
		...
		if (this._isIE6) {
			...
			this.idPull[m] !=
				null && (document.body.removeChild(this.idPull[m]), delete this.idPull[m])
		}
...
dhtmlXMenuObject.prototype._getAllParents = function (b) {
	var c = [], a;
	for (a in this.itemPull)if (this.itemPull[a].parent == b && (c[c.length] = this.itemPull[a].id, this.itemPull[a].complex))for (var d = this._getAllParents(this.itemPull[a].id), f = 0; f < d.length; f++)c[c.length] = d[f];
	return c
};
...
dhtmlXMenuObject.prototype.addContextZone = function (b) {
	if (b == document.body)var b = "document.body." + this.idPrefix, c = document.body; else c = document.getElementById(b);
	var a = !1, d;
	...
	}, c.addEventListener("mouseup", this.operaContext, !1); else {
...
dhtmlXMenuObject.prototype.getItemPosition = function (b) {
	...
	for (var a = this.itemPull[b].parent, d = this.idPull["polygon_" + a] != null ? this.idPull["polygon_" + a].tbd : this.cont, f = 0; f < d.childNodes.length; f++)if (d.childNodes[f] == this.idPull["separator_" + b] || d.childNodes[f] == this.idPull[b])c = f;
	return c
};
...
dhtmlXMenuObject.prototype.addNewChild = function (b, c, a, d, f, e, g) {
	if (b == null)if (this.context)b = this.topId; else {
...
dhtmlXMenuObject.prototype._addItemIntoGlobalStrorage = function (b, c, a, d, f, e, g) {
	var h = {id: b, title: a, imgen: e != null ? e : "", imgdis: g != null ? g : "", type: d, state: f == !0 ? "disabled" : "enabled", parent: c, complex: !1, hotkey: "", tip: ""};
...
dhtmlXMenuObject.prototype._renderSublevelItem = function (b, c) {
	...
	d.onclick = function (b) {
		if (a.checkEvent("onClick") || !a.itemPull[this.id].complex) {
			...
			b.cancelBubble = !0;
			b.returnValue = !1;
			tc = a.itemPull[this.id].complex ?
				"c" : "-";
			td = a.itemPull[this.id].state == "enabled" ? "-" : "d";
			var c = {ctrl: b.ctrlKey, alt: b.altKey, shift: b.shiftKey};
	...
	c != null && (c++, c < 0 && (c = 0), c > p.tbd.childNodes.length -
		1 && (c = null));
...
dhtmlXMenuObject.prototype._renderSeparator = function (b, c) {
	...
	if (!(a == "TopLevel" && this.context)) {
		...
		g.onclick = function (a) {
			a = a || event;
			a.cancelBubble = !0;
			var b = {ctrl: a.ctrlKey, alt: a.altKey, shift: a.shiftKey};
			d._doOnClick(this.id.replace("separator_" + d.idPrefix, ""), "--s", b)
		};
		if (a == "TopLevel") {
			if (c != null)c++, c < 0 && (c = 0), this.cont.childNodes[c] != null ? this.cont.insertBefore(g, this.cont.childNodes[c]) : this.cont.appendChild(g); else {
...
dhtmlXMenuObject.prototype.unload = function () {
	...
	if (this.context)for (var b in this.contextZones)this.removeContextZone(b);
...
dhtmlXMenuObject.prototype.i18n = {dhxmenuextalert: "dhtmlxmenu_ext.js required"};
(function () {
	dhtmlx.extend_api("dhtmlXMenuObject", {_init: function (b) {
		return[b.parent, b.skin]
	}, align: "setAlign", top_text: "setTopText", context: "renderAsContextMenu", icon_path: "setIconsPath", open_mode: "setOpenMode", rtl: "setRTL", skin: "setSkin", dynamic: "enableDynamicLoading", xml: "loadXML", items: "items", overflow: "setOverflowHeight"}, {items: function (b, c) {
