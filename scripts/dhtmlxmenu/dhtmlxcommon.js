...
dhtmlx.extend_api = function (a, b, c) {
	...
	if (d)window[a] = function (a) {
		if (a && typeof a == "object" && !a.tagName) {
			...
			for (f in a)if (b[f])this[b[f]](a[f]); else f.indexOf("on") == 0 && this.attachEvent(f, a[f])
...
dtmlXMLLoaderObject.prototype.getXMLTopNode = function (a, b) {
	...
	if (_isIE && !this._retry) {
		var e = this.xmlDoc.responseText, b = this.xmlDoc;
		this._retry = !0;
		this.xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
		this.xmlDoc.async = !1;
		this.xmlDoc.loadXML(e);
		return this.getXMLTopNode(a,
			b)
	}
...
function getOffsetRect(a) {
	var b = a.getBoundingClientRect(), c = document.body, d = document.documentElement, e = window.pageYOffset || d.scrollTop || c.scrollTop, g = window.pageXOffset || d.scrollLeft || c.scrollLeft, f = d.clientTop || c.clientTop || 0, h = d.clientLeft || c.clientLeft || 0, i = b.top + e - f, k = b.left + g - h;
	return{top: Math.round(i), left: Math.round(k)}
}
...
dhtmlDragAndDropObject.prototype.calculateFramePosition = function (a) {
	if (window.name) {
		...
		if (a)return c + "_" + d; else this.fx = c;
		this.fy = d
	}
	return"0_0"
};
...
dtmlXMLLoaderObject.prototype.doXPath = function (a, b, c, d) {
	...
	if (_isIE)return b || (b = this.xmlDoc.nodeName ? this.xmlDoc : this.xmlDoc.responseXML), b || dhtmlxError.throwError("LoadXML", "Incorrect XML", [b || this.xmlDoc, this.mainObject]), c != null && b.setProperty("SelectionNamespaces", "xmlns:xsl='" + c + "'"), d == "single" ? b.selectSingleNode(a) : b.selectNodes(a) || []; else {
		var e = b;
...
dtmlXMLLoaderObject.prototype.doXPathOpera = function (a, b) {
	...
	if (!c.length)return[];
	if (c[0] == ".")d = [b]; else if (c[0] == "")d = (this.xmlDoc.responseXML || this.xmlDoc).getElementsByTagName(c[e].replace(/\[[^\]]*\]/g, "")), e++; else return[];
...
dhtmlxEventable = function (a) {
	...
	a.eventCatcher = function (a) {
		var c = [], d = function () {
			for (var d = !0, g = 0; g < c.length; g++)if (c[g] != null)var f = c[g].apply(a, arguments), d = d && f;
			return d
		};
...
(function () {
	var a = dhtmlx.message = function (b, c, d, e) {
		...
		typeof b != "object" && (b = {text: b, type: c, lifetime: d, id: e});
		b.type = b.type || "info";
		b.id = b.id || a.uid();
